...
export class demo1657856621009 implements MigrationInterface {
  name = 'demo1657856621009';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE \`order\` (\`order_id\` int NOT NULL AUTO_INCREMENT, \`status\` int NOT NULL, \`amount\` int NOT NULL, \`address\` varchar(255) NULL, \`phonenumber\` varchar(255) NULL, \`created_at\` datetime NULL DEFAULT CURRENT_TIMESTAMP, \`updated_at\` datetime NULL DEFAULT CURRENT_TIMESTAMP, \`userUserId\` int NULL, \`productProductId\` int NOT NULL, PRIMARY KEY (\`order_id\`, \`productProductId\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`order\` ADD CONSTRAINT \`FK_6eef7dc59686bd91c22c53660ee\` FOREIGN KEY (\`productProductId\`) REFERENCES \`product\`(\`product_id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE \`order\` DROP FOREIGN KEY \`FK_6eef7dc59686bd91c22c53660ee\``,
    );
    await queryRunner.query(
